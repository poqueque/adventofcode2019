...
fun pathCalculator(path1: String, path2: String): Int {
    val moves1 = getMoves(path1)
    val moves2 = getMoves(path2)
    val path1 = getPath(moves1)
    println("Path 1 done")
    val path2 = getPath(moves2)
    println("Path 2 done")
    val intersect = getIntersects(path1, path2)
    println("Intersects: ${intersect.size}")
    return getMinDistance(intersect)
}

fun getMinDistance(intersect: List<Point>): Int {
    var md = Int.MAX_VALUE
    intersect.map {
        val d = abs(it.x) + abs(it.y)
        if (d < md) md = d
    }
    return md
}

fun getPath(moves: List<Move>): List<Point> {
    var x = 0
    var y = 0
    val path = mutableListOf(Point(x, y))
    moves.map {
        when (it.dir) {
            "R" -> for (i in 1..it.length) {
                x++
                path.add(Point(x, y))
            }
            "L" -> for (i in 1..it.length) {
                x--
                path.add(Point(x, y))
            }
            "U" -> for (i in 1..it.length) {
                y++
                path.add(Point(x, y))
            }
            "D" -> for (i in 1..it.length) {
                y--
                path.add(Point(x, y))
            }
        }
...
fun getIntersects(path1: List<Point>, path2: List<Point>): List<Point> {
    val intersect = mutableListOf<Point>()
    var min = Int.MAX_VALUE
    path1.map {
        if (path2.contains(it))
            if (abs(it.x) + abs(it.y) > 0) {
                intersect.add(it)
                print("Steps: ")
                val sum = path1.indexOf(it) + path2.indexOf(it)
                println(sum)
                if (sum < min) min = sum
            }
    }
...
fun getMoves(p1: String): List<Move> {
    val d = p1.split(",")
    return d.map {
        Move(it.substring(0, 1), it.substring(1).toInt())
    }
}

data class Move(val dir: String, val length: Int)
...
//    val p2 = "U62,R66,U55,R34,D71,R55,D58,R83"
    ...
    val minDis = pathCalculator(p1, p2)
    println("MinDistance: $minDis")
}
